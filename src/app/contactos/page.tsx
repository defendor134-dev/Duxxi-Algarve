...
export default function ContactosPage() {
  ...
  const validate = () => {
    ...
    if (formData.name.length < 2) newErrors.name = "Nome muito curto";
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) newErrors.email = "Email inválido";
    if (formData.subject.length < 3) newErrors.subject = "Seleciona um assunto";
    if (formData.message.length < 10) newErrors.message = "Mensagem muito curta (mín. 10 caracteres)";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus("error");
        setStatusMessage(data.message || "Erro ao enviar. Tenta novamente.");
      }
    } catch {
      setStatus("error");
      setStatusMessage("Erro de conexão. Verifica a tua internet.");
    }
    setTimeout(() => {
      setStatus("idle");
      setStatusMessage("");
            ...
            <div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-5">
                  <div>
                    <label htmlFor="name" className="form-label-ultra">Nome</label>
                    <input
                      id="name"
                      type="text"
                      value={formData.name}
                      onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                      className={`form-input-ultra ${errors.name ? "border-ultra-red" : ""}`}
                      placeholder="O teu nome"
                      required
                      aria-invalid={!!errors.name}
                      aria-describedby={errors.name ? "name-error" : undefined}
                    />
                    {errors.name && <p id="name-error" className="form-error-ultra">{errors.name}</p>}
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className={`form-input-ultra ${errors.email ? "border-ultra-red" : ""}`}
                      placeholder="teu@email.com"
                      required
                      aria-invalid={!!errors.email}
                      aria-describedby={errors.email ? "email-error" : undefined}
                    />
                    {errors.email && <p id="email-error" className="form-error-ultra">{errors.email}</p>}
                  </div>
                </div>
                <div>
                  <label htmlFor="subject" className="form-label-ultra">Assunto</label>
                  <select
                    id="subject"
                    value={formData.subject}
                    onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
                    className={`form-select-ultra ${errors.subject ? "border-ultra-red" : ""}`}
                    required
                    aria-invalid={!!errors.subject}
                  >
                    <option value="">Seleciona um assunto</option>
                    <option value="Informações sobre a claque">Informações sobre a claque</option>
                    <option value="Quero ser sócio">Quero ser sócio</option>
                    <option value="Deslocações">Deslocações</option>
                    <option value="Sugestão">Sugestão</option>
                    <option value="Outro">Outro</option>
                  </select>
                  {errors.subject && <p className="form-error-ultra">{errors.subject}</p>}
                </div>
                <div>
                  <label htmlFor="message" className="form-label-ultra">Mensagem</label>
                  <textarea
                    id="message"
                    value={formData.message}
                    onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                    className={`form-textarea-ultra ${errors.message ? "border-ultra-red" : ""}`}
                    placeholder="Escreve a tua mensagem..."
                    required
                    aria-invalid={!!errors.message}
                    aria-describedby={errors.message ? "message-error" : undefined}
                  />
                  {errors.message && <p id="message-error" className="form-error-ultra">{errors.message}</p>}
                </div>

                {statusMessage && (
                  <div className={`px-4 py-3 text-sm font-sans border ${
                    status === "success"
                      ? "bg-ultra-green/10 border-ultra-green/30 text-ultra-green-bright"
                      : "bg-ultra-red/10 border-ultra-red/30 text-ultra-red"
                  }`}>
                    {statusMessage}
                  </div>
                )}
